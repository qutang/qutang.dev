...
var getSeries = function (posts) {
  var seriesNames = posts.map(post => post.meta.series).filter((name, index, arr) => arr.indexOf(name) === index && name !== null && name !== "" && name !== undefined);
  return seriesNames;
}
        ...
        var blogItems = seriesPosts.map(post => {
          return (
            <div className='archive-item-container'>
              <time className='archive-item-date'><a href={"/blog/archive#" + moment(post.meta.date).format("YYYY")}>{moment(post.meta.date).format("MMM DD, YYYY")}</a></time>{"-"}<h3 className='archive-item-title'><a href={post.path}>{post.meta.title}</a></h3>
            </div>
          )
        });
        return (
          <section className='archive-section-container'>
            <h2 id={name} className='archive-section-title'>{name} ({seriesPosts.length})</h2>
            {blogItems}
          </section>
        )
      })
