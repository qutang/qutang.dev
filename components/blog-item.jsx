...
export default ({ post }) => (
    <section className='blog-item-container'>
        ...
        <footer className='blog-item-footer-container'>
            <span className='blog-item-series'><a href={"/blog/series#" + post.meta.series || ""}>{post.meta.series || ""}</a></span>{" | "}
            <time className='blog-item-date'><a href={"/blog/archive#" + moment(post.meta.date).format("YYYY")}>{moment(post.meta.date).format("MMM DD, YYYY")}</a></time>
        </footer>
    </section>
)